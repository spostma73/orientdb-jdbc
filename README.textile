...
This fork was created in attempt to translate H2 SQL grammar into OrientDB compliant SQL grammar in order 
to support Hibernate components along with graph enabled components together in one DB. 
(Also to avoid a separate RDBMS which could get out of sync with OrientDB when backed up at different moments, 
